...
export const EvaluatorProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const fetchAll = async () => {
            try {
                const loadedStory = (await axios.get(`/stories/${storyId}`))
                    .data.payload;
    ...
    useEffect(() => {
        ...
        socket.on('update:stats', (data) => {
            ...
            if (story === storyId) {
                const playerLog = playersLog.find(
                    (player) => player.id === senderId,
                );
                ...
                if (playerLog) {
                    ...
                    if (
                        ...
                        pushNotification(
                            `New evaluation requested by ${name || id}`,
                        );
    ...
    const toProvide = {
        ...
        playersLog,
        pushNotification,
        selectedPlayer: playersLog.find(
            (player) => player.id === focusedPlayer,
        ),
        setFocusedPlayer,
        updatePlayerLog,
        socket,
    ...
    return React.createElement(
        ...
        story ? children : React.createElement('div', null, userMessage),
    );
};
export const useEvaluator = () => {
    ...
    if (value == null) {
        throw new Error(
            'useEvaluator must be used inside an EvaluatorProvider',
        );
    }
