...
function FontVarInput({ onChange, value }) {
  ...
  useEffect(() => {
    Object.values(varNameIcon).map(({ prop, defaultValue }) =>
  ...
  if (value[alignProp.propName] === undefined)
    return <p>Loading defaultValue values...</p>;

  return (
    <div key={id}>
      {/*just shutting up react */}
      <ToggleButtonGroup
        value={getFormats(value)}
        onChange={handleFormat}
        aria-label={'font decorations'}
      >
        {Object.values(varNameIcon).map(
          ({ name, IconComponent, prop, active }) => {
            return (
              <ToggleButton
                value={active}
                aria-label={name}
                key={`${name}-button${shortid.generate()} `}
              >
                <IconComponent />
              </ToggleButton>
            );
      ...
      <ToggleButtonGroup
        ...
        onChange={(event, newValue) =>
          handleAlignment(event, newValue, alignProp.propName)
        }
