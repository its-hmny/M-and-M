...
const Inspector = () => {
  ...
  const {InspectorPaper, InspectorElement } = useStyles();
  ...
  const populateInspector = (iterator, previousPath) => {
    ...
    return (iterator.map((element, index) => {
        ...
        const InspectorSection = (
          <InspectorElement 
            fieldList={[...mandatory, ...optional]} 
            pathToComponent={[...absPath, index]}
          />);

        return (
          <Box key={getComponentName(element) + index} className={InspectorElement}>
            <List className={InspectorElement} >
              ...
              <Collapse in={open[getComponentName(element)+'-'+iterator+'-'+index]} unmountOnExit>  
                <Box borderLeft={1} borderColor="primary.main">
                  {InspectorSection}
                  { element.children instanceof Array ? populateInspector(element.children, [...absPath, index, "children"]) : undefined }
                </Box>
              </Collapse>
  ...
  return (
      <Paper className={InspectorPaper} elevation={3}>
        { (!isVisible) ? 
