...
import { Avatar, Tab, Tabs, Grid } from '@material-ui/core';
...
const PlayerSidebar = () => {
  ...
  const generateTab = iterator => 
    iterator.map(playerName => {
      const getInitials = playerName => playerName.split(' ').map(n => n[0]).join('');
      return (
        <Tab label={playerName} icon={<Avatar>{getInitials(playerName)}</Avatar>} />
      );
    });
