...
function ImagePropInput({ onChange, value }) {
  ...
  function getValue(propName) {
    ...
    else returned = propDefaults[propName].replace('%', '');
    return +returned; // so that it returns the int
  }

  useEffect(() => {
    Object.entries(auto ? autoSettings : defaultSettings).map(([key, value]) =>
      onChange(key, value)
    );
  }, [auto, onChange]);
    ...
    <>
      ...
      <Slider
        ...
        onChange={(event, newValue) =>
          handleSlider(event, newValue, propNames.width)
        }
      />
      ...
      <Slider
        ...
        valueLabelDisplay="on"
        onChange={(event, newValue) =>
          handleSlider(event, newValue, propNames.height)
        }
        style={{ height: '300px' }}
      />
    </>
